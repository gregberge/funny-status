...
Daemon.prototype.loop = function () {
  this.pinger.ping(function () {
    forEach(this.pinger.history, this.handleStatusChange.bind(this));
...
Daemon.prototype.handleStatusChange = function (history) {
  var change = st.getChange(history);
  if (change === 'up') this.options.success();
  if (change === 'down') this.options.failure();
};
